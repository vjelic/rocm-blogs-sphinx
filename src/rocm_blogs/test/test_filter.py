...
def test_filter_criteria():
    ...
    if not os.path.exists(blogs_directory):
        blogs_directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), "test_blogs")
        if not os.path.exists(blogs_directory):
    ...
    pagination_template = "<div>{prev_button} Page {current_page} of {total_pages} {next_button}</div>"
